...
class AuthJwtValidator(models.Model):
    ...
    user_id_strategy = fields.Selection(
        [("static", "Static"), ("email", "Email")], required=True, default="static"
    )
    ...
    partner_id_required = fields.Boolean(default=True)
    ...
    def _onchange_user_id_strategy(self):
        if self.user_id_strategy == "email":
            self.partner_id_required = True
    ...
    def _encode(self, payload, secret, expire):
        ...
        return self._get_jwt_token(payload, secret)
    ...
    def _get_uid(self, partner_id):
        # override for additional strategies
        if self.user_id_strategy == "static":
            return self.static_user_id.id
        elif self.user_id_strategy == "email":
            partner = self.env["res.partner"].browse(partner_id)
            return (
                partner.user_ids.ids[0]
                if partner.user_ids
                else self.env.ref("base.public_user").id
            )

    def _get_jwt_token(self, payload, api_license):
        return jwt.encode(payload, key=api_license, algorithm="HS256")

    def _get_and_check_uid(self, partner_id):
        uid = self._get_uid(partner_id)
        if not uid:
    ...
    def _get_company_id(self, payload):
        company_registry = payload.get("company_registry")
        if not company_registry:
            _logger.debug("JWT payload does not have an company_registry claim")
            return
        ...
        if len(company_id) != 1:
            _logger.debug(
                "%d company found for company_registry %s",
                len(company_id),
                company_registry,
            )
            return
    ...
    def _get_and_check_partner_id(self, payload):
        ...
        company_id = self._get_company_id(payload)
    ...
    def _create_record_request_log(
        self, request, payload, uid, client_ip, request_id, partner_id, body
    ):
        data = self.prepare_request_history(
            request, payload, uid, client_ip, request_id, partner_id, body
        )
        request.env["auth.jwt.request"].sudo().create(data)

    def prepare_request_history(
        self, request, payload, uid, client_ip, request_id, partner_id, body
    ):
        country_id = (
            request.env["res.country"]
            .sudo()
            .search([("code", "=", request.geoip.get("country_code"))])
        )
        return {
            "name": _("API Request #%s", request_id),
    ...
    def get_response_status(self, code):
        # List of response code and message
        # this list use in standard response
        response = {
            200: "Request successful",
            201: "Data created successfully",
            204: "No content found",
            206: "Some data failed to create",
            400: "Bad request",
        ...
        return response.get(code, "Invalid code")
