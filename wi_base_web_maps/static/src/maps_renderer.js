...
const googleApiTilesRoute = "http://{s}.google.com/vt/lyrs=s,h&x={x}&y={y}&z={z}";
...
const mapTileAttribution = `
    © <a href="https://www.google.com/maps/about/">Google Maps</a>
    `;

export class MapsRenderer extends Component {
    setup() {
        ...
        useEffect(
            () => {
                ...
                L.tileLayer(googleApiTilesRoute, {
    ...
    addMarkers() {
        ...
        if (this.props.model.data.isGrouped) {
            records = Object.entries(this.props.model.data.recordGroups)
                ...
                .flatMap(([groupId, value]) =>
                    value.records.map((elem) => ({...elem, groupId}))
                );
        }
        ...
        for (const record of records) {
            ...
            if (location && location.location_latitude && location.location_longitude) {
                const lat_long = `${location.location_latitude}-${location.location_longitude}`;
                const group = this.props.model.data.recordGroups
                    ? `-${record.groupId}`
                    : "";
        ...
        for (const markerInfo of Object.values(markersInfo)) {
            ...
            // Icon creation
            const iconInfo = {
                className: "o-maps-renderer--marker",
                html: renderToString("wi_base_web_maps.marker", params),
            };
            ...
            const marker = L.marker(
                [
                    markerInfo.record.location.location_latitude + offset,
                    markerInfo.record.location.location_longitude - offset,
                ],
    ...
    addRoutes() {
        this.removeRoutes();
        if (
            !this.props.model.data.useMapBoxAPI ||
            !this.props.model.data.routes.length
        ) {
            return;
        }
    ...
    createMarkerPopup(markerInfo, latLongOffset = 0) {
        ...
        const location = markerInfo.record.location;
        const coordinate =
            location.location_latitude + "," + location.location_longitude;
        const popupHtml = renderToString("wi_base_web_maps.markerPopup", {
            fields: popupFields,
            hasFormView: this.props.model.metaData.hasFormView,
            url: `https://www.google.com/maps/dir/?api=1&destination=${coordinate}`,
        });
        const popup = L.popup({offset: [0, -30]})
            .setLatLng([
                location.location_latitude + latLongOffset,
                location.location_longitude - latLongOffset,
            ])
            .setContent(popupHtml)
            .openOn(this.leafletMap);
        const openBtn = popup
            .getElement()
            .querySelector("button.o-maps-renderer--popup-buttons-open");
        if (openBtn) {
            openBtn.onclick = () => {
    ...
    getMarkerPopupFields(markerInfo) {
        ...
        if (markerInfo.ids.length > 1) {
            if (!this.props.model.metaData.hideAddress) {
                fieldsView.push({
                    id: this.nextId++,
                    value: record.location.contact_address_complete,
                    string: _t("Address"),
                });
            }
            return fieldsView;
        ...
        for (const field of this.props.model.metaData.fieldNamesMarkerPopup) {
            if (record[field.fieldName]) {
                ...
                if (fields[field.fieldName].type === "many2one") {
                    ...
                    ["one2many", "many2many"].includes(fields[field.fieldName].type)
                ) {
    ...
    get googleMapUrl() {
        ...
        if (this.props.model.data.records.length) {
            ...
            const uniqueCoordinates = allCoordinates.reduce((coords, {location}) => {
                const coord =
                    location.location_latitude + "," + location.location_longitude;
...
MapsRenderer.template = "wi_base_web_maps.MapsRenderer";
MapsRenderer.props = {
    model: Object,
